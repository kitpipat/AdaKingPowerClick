                    ...
                    <tbody>
                        <?php if (isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])) { ?>
                            ...
                            <?php foreach ($aDataReport['aRptData'] as $nKey => $aValue) { ?>
                                ...
                                <?php if($tBchCode != $tBchCodeNew){ ?>
                                    <tr>
                                       <?php if($tBchCode == ''){
                                           $tBCHCodeShow = 'ไม่พบสาขา';
                                       }else{
                                           $tBCHCodeShow = language('report/report/report', 'tRptTaxSalePosBch') . '(' . $aValue["FTBchCode"] . ') ' . $aValue["FTBchName"];
                                       } ?>
                                       <td nowrap class="text-left xCNRptGrouPing" style="padding-left: 50px !important;" colspan="4"><?=$tBCHCodeShow?> </td>
                                   </tr>
                                <?php $tBchCodeNew = $tBchCode; } ?>

                                <tr>
                                    <?php if($aValue["FTWahCode"] == ''){
                                        $tWAHCode = 'ไม่พบคลัง';
                                    }else{
                                        $tWAHCode = '(' . $aValue["FTWahCode"] . ') ' . $aValue["FTWahName"];
                                    } ?>
                                    <td nowrap class="text-left xCNRptDetail" style="padding-left: 50px !important;" colspan="2">&nbsp;&nbsp;&nbsp;&nbsp; <?=$tWAHCode?></td>
