...
class RptSalesDailyByCashierKPC_model extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter) {
        ...
        $tCallStore = "{ CALL SP_RPTxSalDailyByCashierKPC(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
            'pnLngID'       => $nLangID,
            'ptComName'     => $tComName,
            'ptRptCode'     => $tRptCode,
    ...
    public function FSaMGetDataReport($paDataWhere) {
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT  
                    ...
                    S.FNRptGroupMember,
                    S.FCXshNet_SubTotal,
                    CASE WHEN ISNULL(RCV.FTRcvCode,'') = '' THEN '2' ELSE '1' END AS FTStaShw
                FROM TRPTSalDailyByCashierKPCTmp A WITH(NOLOCK)
                LEFT JOIN TFNMRcv RCV ON A.FTRcvCode = RCV.FTRcvCode AND RCV.FTFmtCode IN ('002','004')
                LEFT JOIN (
                    SELECT
                        FTRcvCode AS FTRcvCode_SUM,
                        COUNT(FTRcvCode) AS FNRptGroupMember,
                        SUM(FCXshNet) AS FCXshNet_SubTotal
                    FROM TRPTSalDailyByCashierKPCTmp WITH(NOLOCK)
        ...
        $tSQL .= " ORDER BY L.FTUsrCode, L.FTTnsType, L.FTRcvCode ASC,L.FTRcvRefNo1 ASC ";
    ...
    public function FMxMRPTSetPriorityGroup($ptComName, $ptRptCode, $ptUsrSession) {
        ...
        $tSQL = "   UPDATE TRPTSalDailyByCashierKPCTmp SET 
                        FNRowPartID = B.PartID,
                        FNRcvUseAmtSum = B.UseAmt,
                        FNRcvUseAmtSumSub = B.UseAmtSub
                    FROM( 
                        SELECT 
                            ROW_NUMBER() OVER(PARTITION BY FTUsrCode ORDER BY FTUsrCode,FTTnsType,FTRcvCode ASC) AS PartID,
                            SUM(FNRcvUseAmt) OVER(PARTITION BY FTUsrCode ORDER BY FTUsrCode ASC) AS UseAmt,
                            SUM(FNRcvUseAmt) OVER(PARTITION BY FTUsrCode,FTRcvCode ORDER BY FTUsrCode,FTRcvCode ASC) AS UseAmtSub,

                            FNRptRowSeq
                        FROM TRPTSalDailyByCashierKPCTmp TMP WITH(NOLOCK)
                        WHERE TMP.FTComName		= '$ptComName' 
                        AND TMP.FTRptCode		= '$ptRptCode'
                        AND TMP.FTUsrSession	= '$ptUsrSession' 
                    ) AS B
                    WHERE TRPTSalDailyByCashierKPCTmp.FNRptRowSeq	= B.FNRptRowSeq 
                    AND TRPTSalDailyByCashierKPCTmp.FTComName		= '$ptComName' 
                    AND TRPTSalDailyByCashierKPCTmp.FTRptCode		= '$ptRptCode'
                    AND TRPTSalDailyByCashierKPCTmp.FTUsrSession	= '$ptUsrSession'
        ";
