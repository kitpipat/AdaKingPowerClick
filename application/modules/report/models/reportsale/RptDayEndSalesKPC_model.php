...
class RptDayEndSalesKPC_model extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore     = "{ CALL SP_RPTxSalDailyKPC(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " LEFT JOIN (
                                    SELECT
                                        FTUsrSession AS FTUsrSession_Footer,
                                        SUM ( ISNULL(FCXshGrand, 0 ) ) AS FCXshGrand_Footer,
                                        SUM ( ISNULL(FCXshGrand, 0 ) ) AS FNRcvUseAmtUse_Footer,
                                        SUM ( ISNULL(FNRcvUseAmt, 0 ) ) AS FNRcvUseAmtTotal_Footer
                                    FROM TRPTSalDailyKPCTmp WITH(NOLOCK)
            ...
            $tRptJoinFooter = " LEFT JOIN (
                                    SELECT
                                        ...
                                        0 AS FNRcvUseAmtUse_Footer,
                                        0 AS FNRcvUseAmtTotal_Footer
                                ) T ON  L.FTUsrSession = T.FTUsrSession_Footer
            ";
        }
        ...
        $tSQL   =   "   
                ...
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY FTRcvName ORDER BY FTXihValType ASC ,FTRcvCode ASC) AS RowID,
                        A.*,
                        S.FNRptGroupMember,
                        B.FNBillSalTotal
                    FROM TRPTSalDailyKPCTmp A WITH(NOLOCK)
                    LEFT JOIN (
                        SELECT
                    ...
                    LEFT JOIN (
                        SELECT
                            FTUsrSession,
                            FNRcvUseAmt AS FNBillSalTotal
                        FROM TRPTSalDailyKPCTmp WITH(NOLOCK)
                        WHERE FTComName = '$tComName'
                        AND FTRptCode = '$tRptCode'
                        AND FTUsrSession = '$tUsrSession'
                        AND FTXihValType = '1'
                    ) AS B ON A.FTUsrSession = B.FTUsrSession
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL = "   
            UPDATE TRPTSalDailyKPCTmp SET 
            TRPTSalDailyKPCTmp.FNRowPartID = B.PartID
                FROM(
                    SELECT   
                    ROW_NUMBER() OVER(PARTITION BY TMP.FTXihValType ORDER BY TMP.FTXihValType ASC) AS PartID ,
                        TMP.FTRptRowSeq
                    FROM TRPTSalDailyKPCTmp TMP WITH(NOLOCK)
                    WHERE TMP.FTComName  = '$tComName'
                    AND TMP.FTRptCode    = '$tRptCode'
                    AND TMP.FTUsrSession = '$tUsrSession'
                ) AS B
            WHERE TRPTSalDailyKPCTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTSalDailyKPCTmp.FTComName = '$tComName' 
            AND TRPTSalDailyKPCTmp.FTRptCode = '$tRptCode'
            AND TRPTSalDailyKPCTmp.FTUsrSession = '$tUsrSession'
        ";
    }
    ...
    public function FSnMCountDataReportAll($paDataWhere)
    {
        ...
        $tSQL = "   SELECT 
                    ...
                    FROM TRPTSalDailyKPCTmp AS DTTMP WITH(NOLOCK)
                    WHERE 1 = 1
