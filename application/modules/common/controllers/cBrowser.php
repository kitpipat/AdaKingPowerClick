...
class cBrowser extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($oPtions != '' || $oPtions != 'undefined') :
            ...
            $nPerPage = 10;
            // Page Option
            if( isset($oPtions['GrideView']['Perpage']) && $oPtions['GrideView']['Perpage'] > 10 ){
            ...
            $tFilerGride = trim($this->input->post('tFilerGride')); // Filter Value
            // Filter Data From Filter Element
            if (isset($tFilerGride)) {
                if ($tFilerGride != '') {
                    //mos 21/01/2020 กรณีถ้าฟิวส์เป็นตัวเลข มันไม่ต้องใส่ collate thai_bin
                    if( isset($tColPk) && !empty($tColPk) ){
                        $aColum = explode(".",$tColPk);
                        if (strpos($aColum[1],'FN') !== false) {
                            $tSQL .= " AND ( $tColPk COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                        }else if(strpos($aColum[1],'FD') !== false){
                            $tSQL .= " AND ( CONVERT(VARCHAR(10), $tColPk, 121) LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                        }else {
                            $tSQL .= " AND ( $tColPk LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                        }
                    }
                    ...
                    for ($fc = 0; $fc < FCNnHSizeOf($oPtions['GrideView']['DataColumns']); $fc++) {
                        ...
                        //mos 21/01/2020 กรณีถ้าฟิวส์เป็นตัวเลข มันไม่ต้องใส่ collate thai_bin
                        if( isset($tFilterCol) && !empty($tFilterCol) ){
                            $aColum = explode(".",$tFilterCol);
                            if (strpos($aColum[1],'FN') !== false) {
                                $tSQL .= "  OR $tFilterCol COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                            } else if (strpos($aColum[1],'FD') !== false) {
                                $tSQL .= "  OR CONVERT(VARCHAR(10), $tFilterCol, 121) LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                            } else {
                                $tSQL .= "  OR $tFilterCol LIKE '%".$this->db->escape_like_str($tFilerGride)."%' ";
                            }
                        }
                    }
                    $tSQL .= " ) ";
                ...
                foreach ($aDataTable as $key => $val) :
                    ...
                    if ($tIDCurrent != $val->$tMasterPK) :
                        ...
                        elseif ($CallBackType == 'M') :
                            ...
                            $tDataTable .= '<tr class="xCNTextDetail2 ' . $tRowActive . ' " onclick="JCNxPushMultiSelection(' . "'" . $val->$tMasterPK . "'," . htmlspecialchars(json_encode($tCallBackTextData), ENT_QUOTES, "UTF-8") . ",this" . ')">';
                        else :
                        ...
                        if (isset($oPtions['NextFunc']['ArgReturn'])) :
                            ...
                            $tDataTable .= "<input type='text' class='xCNKeepValue".$val->$tMasterPK."' style='display:none' id='ohdCallBackArg" . $val->$tMasterPK . $tMasterTable . "' value='" . htmlspecialchars(json_encode($aArgRet), ENT_QUOTES, "UTF-8") . "'" . ">";
                        endif;

                        $tDataTable .= "<input type='text' style='display:none' id='ohdCallBackText" . $val->$tMasterPK . $tMasterTable . "' value='" . htmlspecialchars($val->$tCallBackColumn, ENT_QUOTES, "UTF-8") . "'" . ">";
