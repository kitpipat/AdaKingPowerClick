...
class mProducttransferbranch extends CI_Model {
    ...
    public function FSaMTBXList($paData){
        ...
        if($this->session->userdata("tSesUsrLevel") != "HQ" ){
            $tWhereBchCodeUsr = $this->session->userdata("tSesUsrBchCodeMulti");
            $tSQL .= " AND TBX.FTBchCode IN ($tWhereBchCodeUsr) ";
        }
        ...
        if (!empty($tSearchStaDoc) && ($tSearchStaDoc != "0")) {
            if ($tSearchStaDoc == 3) {
                ...
                $tSQL .= " AND ISNULL(TBX.FTXthStaApv,'') = '' AND TBX.FTXthStaDoc != '3'";
            } elseif ($tSearchStaDoc == 1) {
                $tSQL .= " AND TBX.FTXthStaApv = '$tSearchStaDoc'";
            }
        }
        ...
        if (isset($tSearchStaPrcStk) && !empty($tSearchStaPrcStk)) {
            if ($tSearchStaPrcStk == 3) {
                $tSQL .= " AND (TBX.FTXthStaPrcStk = '$tSearchStaPrcStk' OR ISNULL(TBX.FTXthStaPrcStk,'') = '') ";
            } else {
        ...
        if (!empty($tSearchStaDocAct) && ($tSearchStaDocAct != "0")) {
            if ($tSearchStaDocAct == 1) {
                $tSQL .= " AND TBX.FNXthStaDocAct = 1";
            } else {
                $tSQL .= " AND TBX.FNXthStaDocAct = 0";
            }
        }
        ...
        if($paDataWhere['nTBOptionAddPdt']==1){
            ...
            if($oQuery->num_rows() > 0){
                ...
                $this->db->insert('TCNTDocDTTmp',array(
                    ...
                    'FTTmpStatus'       => $paData['FTPdtForSystem']
                ));
    ...
    function FSaMTBAddUpdateDocNoInDocTemp($aDataWhere){
        ...
        try{
            ...
            $this->db->set('FTXshDocNo' , $aDataWhere['FTXthDocNo']);    
            $this->db->set('FTBchCode'  , $aDataWhere['FTBchCode']);    
    ...
    */
    ...
    public function FSaMTBGetHD($paData){
        ...
        $tSQL = "SELECT
                ...
                LEFT JOIN TCNMBranch_L     BCHLC  WITH (NOLOCK)  ON TBX.FTBchCode    = BCHLC.FTBchCode AND BCHLC.FNLngID = $nLngID
    ...
    public function FSaMTBGetDT($paData){
        ...
        $tSQL   = "SELECT c.* FROM(
                        SELECT  ROW_NUMBER() OVER(ORDER BY FTXthDocNo ASC) AS FNRowID,* FROM
                            ...
                            LEFT JOIN TCNMPdt PDT ON TWFDT.FTPdtCode = PDT.FTPdtCode
                            ";
        ...
        if($paData['rtCode'] == 1){
            ...
            foreach($paData as $key=>$val){
                ...
                $this->db->insert('TCNTDocDTTmp',array(
                    ...
                    'FDCreateOn'        => date('Y-m-d h:i:sa'),
                    'FTCreateBy'        => $this->session->userdata('tSesUsername'),
                    'FTTmpStatus'       => $val['FTPdtForSystem'],
    ...
    public function FSnMTBGetPageAll($paData){
        ...
        if($this->session->userdata("tSesUsrLevel") != "HQ" ){
            $tWhereBchCodeUsr = $this->session->userdata("tSesUsrBchCodeMulti");
            $tSQL .= " AND TB.FTBchCode IN ($tWhereBchCodeUsr) ";
        }
    ...
    public function FSxMClearPdtInTmp(){
        ...
        $tSQL = "DELETE FROM TCNTDocDTTmp WHERE FTSessionID = '$tSesSessionID' AND FTXthDocKey = '$tDocKey'";
        $this->db->query($tSQL);

        $tSQL = "DELETE FROM TCNTDocDTFhnTmp WHERE FTSessionID = '$tSesSessionID' AND FTXthDocKey = '$tDocKey'";
        $this->db->query($tSQL);
    }
    ...
    public function FSnMTBDel($paData){
        try{
            ...
            $this->db->where_in('FTXthDocNo', $paData['FTXthDocNo']);
            $this->db->delete('TCNTPdtTwxDTFhn');
