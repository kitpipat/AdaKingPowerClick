...
class mProducttransferwahouse extends CI_Model
{
    ...
    public function FSaMTFWList($paData)
    {
        ...
        $tSQL   =   "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC, FTXthDocNo DESC) AS FNRowID,*
                FROM( 
                    SELECT DISTINCT
                        TFW.FTBchCode,
                        BCHL.FTBchName,
                    ...
                    LEFT JOIN TCNMBranch_L  BCHL WITH (NOLOCK) ON TFW.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID   = ".$this->db->escape($nLngID)."
                    LEFT JOIN TCNMUser_L    USRL WITH (NOLOCK) ON TFW.FTCreateBy = USRL.FTUsrCode AND USRL.FNLngID  = ".$this->db->escape($nLngID)."
                    LEFT JOIN TCNMUser_L    USRLAPV WITH (NOLOCK) ON TFW.FTXthApvCode = USRLAPV.FTUsrCode AND USRLAPV.FNLngID = ".$this->db->escape($nLngID)."
                    WHERE TFW.FDCreateOn <> ''
        ";
        ...
        if (@$tSearchList != '') {
            $tSQL   .= " 
                AND (
                    (TFW.FTXthDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (CONVERT(CHAR(10),TFW.FDXthDocDate,103) LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                )
            ";
        }
        ...
        if ($this->session->userdata("tSesUsrLevel") == "HQ") {
            if (!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeTo)) {
                $tSQL   .= "
                    AND (
                        (TFW.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeFrom)."  AND ".$this->db->escape($tSearchBchCodeTo).")
                        OR (TFW.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeTo)." AND ".$this->db->escape($tSearchBchCodeFrom).")
                    )
                ";
            }
        } else {
        ...
        if (!empty($tSearchDocDateFrom) && !empty($tSearchDocDateTo)) {
            $tSQL   .= " 
                AND (
                    (TFW.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00").")  AND CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:59:59")."))
                    OR (TFW.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:00:00").") AND CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00")."))
                )
            ";
        }
        ...
        if (!empty($tSearchStaDoc) && ($tSearchStaDoc != "0")) {
            if ($tSearchStaDoc == 3) {
                $tSQL   .= " AND TFW.FTXthStaDoc = ".$this->db->escape($tSearchStaDoc)." ";
            } else if ($tSearchStaDoc == 2) {
                $tSQL   .= " AND ISNULL(TFW.FTXthStaApv,'') = '' AND TFW.FTXthStaDoc != '3'";
            } else if ($tSearchStaDoc == 1) {
                $tSQL   .= " AND TFW.FTXthStaApv = ".$this->db->escape($tSearchStaDoc)." ";
            }
        }
        ...
        if (!empty($tSearchStaDocAct) && ($tSearchStaDocAct != "0")) {
            if ($tSearchStaDocAct == 2) {
                ...
                $tSQL   .= " AND TFW.FNXthStaDocAct = ".$this->db->escape($tSearchStaDocAct)." ";
            }
        }
...
// echo $tSQL;
    ...
    public function FSnMTFWGetPageAll($paData)
    {
        ...
        $nLngID         = $paData['FNLngID'];
        $tSQL           = "
            SELECT COUNT (TFW.FTXthDocNo) AS counts
            FROM [TCNTPdtTwxHD] TFW WITH (NOLOCK)
            LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON TFW.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($nLngID)." 
            WHERE TFW.FDCreateOn <> ''
        ";
        ...
        if (@$tSearchList != '') {
            $tSQL   .= " 
                AND (
                    (TFW.FTXthDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (TFW.FDXthDocDate LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                )
            ";
        }
    ...
    public function FSaMTFWAddUpdateHDRef($paData)
    {
        ...
        try {
            ...
            $this->db->set('FTCarCode', $paData['FTCarCode']);
