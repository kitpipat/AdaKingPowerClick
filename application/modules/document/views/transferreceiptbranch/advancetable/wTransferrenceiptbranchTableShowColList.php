<div style="max-height:350px;overflow:auto;">
    <table class="table table-bordered table-striped" id="otbPIOrderListDetail">
        <thead>
            <tr class="xCNCenter">
            ...
            <?php if(isset($aAvailableColumn) && !empty($aAvailableColumn)):?>
                ...
                <?php foreach($aAvailableColumn as $nAdvTblKey => $aDataAdvTbl):?>
                    ...
                    <?php if($aDataAdvTbl->FTShwNameUsr != '' && $aDataAdvTbl->FTShwFedShw != ""): ?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><label contenteditable="true" class="xWTWILabelColumnName"><?php echo $aDataAdvTbl->FTShwNameUsr;?></label></td>
                            <td class="text-center">
                                <input class="xWTWIInputColStaShow" type="checkbox" <?php echo $tChecked?> data-id="<?php echo $aDataAdvTbl->FTShwFedShw;?>">
                            </td>
                        </tr>
                        <?php $i++; ?>
                    <?php endif; ?>
...
<div style="padding:10px 0px;"><input type="checkbox" id="ocbTWISetDefAdvTable"> <?php echo language('common/main/main','tModalAdvUseDefOption')?></div>
<script src="<?php echo base_url(); ?>application/modules/common/assets/js/jquery.tablednd.js"></script>
<script type="text/javascript">
    $(document).ready(function() {
        $("#otbPIOrderListDetail").tableDnD();
    });
</script>
