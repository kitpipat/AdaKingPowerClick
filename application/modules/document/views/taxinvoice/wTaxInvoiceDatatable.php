            ...
            <table class="table table-striped">
                <thead>
                    <tr class="xCNCenter">
                        ...
                        <th class="xCNTextBold"><?=language('document/document/document','tDocPdtSetPrice')?></th>
                    ...
                    <?php if($aGetDT['rtCode'] == 1):?>
                        <?php foreach($aGetDT['raItems'] as $nKey => $aValue): ?>
                            <tr class="text-center xCNTextDetail2">  
                                ...
                                <td class="text-left"><?=$aValue['FTXsdPdtName']?></td>
                                <td class="text-left"><?=$aValue['FTPunName']?></td>
                                <td class="text-right"><?=number_format($aValue['FCXsdQty'],$nOptDecimalShow)?></td>
                                <td class="text-right"><?=number_format($aValue['FCXsdSetPrice'],$nOptDecimalShow)?></td>
                                <td class="text-right"><?=number_format($aValue['FCXsdDis'] + $aValue['FCXsdChg'],$nOptDecimalShow)?></td> <!-- +$aValue['DISPMT'] -->
                                <td class="text-right"><?=number_format($aValue['FCXsdNet'],$nOptDecimalShow)?></td> <!-- - $aValue['DISPMT'] -->
                            </tr>
                        <?php endforeach;?>
                    <?php else:?>
...
<!--Page-->
<?php if($aGetDT['rtCode'] == 1):?>
    <?php if(FCNnHSizeOf($aGetDT['raItems']) > 0): ?>
        <div class="row">
