                    ...
                    <?php if($aDataList['rtCode'] == 1 ):?>
                        <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                            <?php
                                ...
                                $aDateDoc = explode("/",$aValue['FDXphDocDate']);
                                $tDateDoc = $aDateDoc[2].'-'.$aDateDoc[1].'-'.$aDateDoc[0];
                            ?>
                            <tr style="cursor:pointer;" class="text-center xCNTextDetail2 xWTRBDocItems xPurchaseInvoiceRefInt" 
                                    id="otrPurchaseInvoiceRefInt<?php echo $nKey?>" 
                                    data-docno="<?php echo $aValue['FTXphDocNo']?>"
                                    data-docdate="<?php echo $tDateDoc ?>"
                                    data-bchcode="<?php echo $tTRBBchCode?>"
                            >
...
<div class="">
    ...
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
        <div class="xWTRBPageDataTable btn-toolbar pull-right">
            <?php if($nPage == 1){ $tDisabledLeft = 'disabled'; }else{ $tDisabledLeft = '-';} ?>
            <button onclick="JSvTRBRefIntClickPageList('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>>
            ...
            <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataList['rnAllPage'],$nPage+2)); $i++){?>
                ...
                ?>
                <button onclick="JSvTRBRefIntClickPageList('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
            <?php } ?>

            <?php if($nPage >= $aDataList['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
            <button onclick="JSvTRBRefIntClickPageList('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>>
...
<div class="">
        <div id="odvTRBRefIntDocDetail">

        </div>
</div>

<?php include('script/jTransferRequestBranchRefDocDataTable.php')?>
