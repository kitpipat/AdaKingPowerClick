                        ...
                        <td nowrap>
                            <?php
                                switch($aValue['FTXthRefKey']){
                                    case 'DocEntry':
                                        echo "ใบขอเบิกสินค้าออก(GI Draft)";
                                        break;
                                    case 'DocNum':
                                        echo "ใบขอเบิกสินค้าออก(GI Draft)";
                                        break;
...
<script>
    ...
    $('.xWEditDocRef').off('click').on('click',function(){
        ...
        if( nAlwDel != "2" ){
            $('#ocbTWORefType').val(tRefType);
            $('#ocbTWORefType').selectpicker('refresh');
            $('#oetTWORefDocDate').datepicker({ dateFormat: 'yy-mm-dd' }).val(tRefDocDate);

            if(tRefType == 1){//ภายใน
                $('#oetTWODocRefIntName').val(tRefDocNo);
                $('#oetTWODocRefInt').val(tRefDocNo);
            }else{ //ภายนอก
                $('#oetTWORefDocNo').val(tRefDocNo);
            }

            $('#oetTWORefKey').val(tRefKey);
            $('#oetTWORefDocNoOld').val(tRefDocNo);
