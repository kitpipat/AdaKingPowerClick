...
function JSnSaveDTEdit(paEvent) {
    ...
    $(".xWValueEditInLine" + tEditSeqNo).each(function(index) {
        ...
        if (tValue < 1) {
            $('#odvPDTTrnCantQtyisZero').modal('toggle');
            tValue = 1;
        }
...
function FSvTFWEditPdtIntoTableDT(ptEditSeqNo, paField, paValue) {
    var ptXthDocNo      = $("#oetXthDocNo").val();
    var ptPageCurrent   = $("#ohdTWOPageCurrent").val();
    ...
    $.ajax({
        ...
        success: function(tResult) {
            ...
            JSvTFWLoadPdtDataTableHtml(ptPageCurrent);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
        }
...
function JSxSubmitEventByButton() {
    ...
    $(".xCNControllForm").removeAttr("disabled", true);
    $(".ocbListItem").removeAttr("disabled", true);
    $(".xCNBtnBrowseAddOn").removeAttr("disabled", true);
    $(".xCNDocBrowsePdt").removeAttr("disabled", true).removeClass("xCNBrowsePdtdisabled");
    $("#oetSearchPdtHTML").removeAttr("disabled", false);
    $("#otaTfwRmk").removeAttr("disabled", false);
    $.ajax({
        type: "POST",
...
function JCNxTFWControlObjAndBtn() {
    ...
    if (ohdXthStaApv == 1) {
        ...
        $(".xCNDocBrowsePdt").hide();
        // =========== Edit By Witsarut 28/08/2019 ===========


        //Control input ปิด
        $(".xCNControllForm").attr("disabled", true);
        ...
        $("#obtTFWBrowseShipAdd").attr("disabled", true);
    }
...
//จากเอกสารใบเติมสินค้าแบบชุด -> เอกสารใบโอนสินค้าระหว่างคลัง (วันที่ : 23/11/2020)
function JSxRefillVDToTransferWahousePageEdit(ptXthDocNo) {
    $.ajax({
        type: "POST",
        url: "TFWPageEdit",
        data: { ptXthDocNo: ptXthDocNo },
