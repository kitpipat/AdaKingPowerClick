...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[SP_RPTxPSSDailyBySChn]') AND type in (N'P', N'PC'))
	DROP PROCEDURE SP_RPTxPSSDailyBySChn
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_RPTxPSSDailyBySChn] 
	@pnLngID int , 
	@pnComName Varchar(100),
	@ptRptCode Varchar(100),
	@ptUsrSession Varchar(255),
...
-- Watcharakorn 
-- Create 06/11/2020
-- Last Update : Napat(Jame) 30/08/2022 แก้ไขกรณีโยนพารามิเตอร์วันที่มา
...
BEGIN TRY
	...
	SET @FNResult= 0
	--NUI 08-01-2020
	SET @tDocDateF = CONVERT(VARCHAR(10),@ptDocDateF,121)
	SET @tDocDateT = CONVERT(VARCHAR(10),@ptDocDateT,121)
	...
	SET @tSql2 =   ' '
		...
		BEGIN
			...
			SET @tSql2 +=' AND Bch.FTAgnCode IN (' + @ptAgnL + ')'
		END
		...
		BEGIN
			...
			SET @tSql2 += ' AND HD.FTPosCode IN ('+@ptPosCodeL+ ')'
		END		
	--END
	--รหัสช่องทางการขาย
	IF (@ptChnF <> '' AND @ptChnT <> '')
	BEGIN
		SET @tSql1 +=' AND HD.FTChnCode BETWEEN ''' + @ptChnF + ''' AND ''' + @ptChnT + ''''
	END
	--รหัสลูกค้า
	IF (@ptCstF <> '' AND @ptCstT <> '')
	BEGIN
		SET @tSql1 +=' AND HD.FTCstCode BETWEEN ''' + @ptCstF + ''' AND ''' + @ptCstT + ''''
		SET @tSql2 +=' AND HD.FTCstCode BETWEEN ''' + @ptCstF + ''' AND ''' + @ptCstT + ''''
	END

	IF (@tDocDateF <> '' AND @tDocDateT <> '')
	BEGIN
		SET @tSql1 +=' AND CONVERT(VARCHAR(10),HD.FDXshDocDate,121) BETWEEN ''' + @tDocDateF + ''' AND ''' + @tDocDateT + ''''
		SET @tSql2 +=' AND CONVERT(VARCHAR(10),HD.FDXshDocDate,121) BETWEEN ''' + @tDocDateF + ''' AND ''' + @tDocDateT + ''''
	END


	--PRINT  @tSql1
	--PRINT  @tSql2
	DELETE FROM TRPTPSSDailyBySChnTmp WITH (ROWLOCK) WHERE FTComName =  '' + @nComName + ''  AND FTRptCode = '' + @tRptCode + '' AND FTUsrSession = '' + @tUsrSession + ''--ลบข้อมูล Temp ของเครื่องที่จะบันทึกขอมูลลง Temp
	--sale
	SET @tSql =' MERGE INTO TRPTPSSDailyBySChnTmp AS SalTGT'
	SET @tSql +=' USING('
				SET @tSql +=' SELECT '''+ @nComName + ''' AS FTComName,'''+ @tRptCode +''' AS FTRptCode, '''+ @tUsrSession +''' AS FTUsrSession,'	
				SET @tSql +=' 1 AS FNAppType,HD.FTBchCode,BchL.FTBchName,HD.FTPosCode,CONVERT(DATE,FDXshDocDate,208) AS FDXshDocDate,'
				SET @tSql +=' HD.FTXshDocNo,FTXshRefInt,HD.FTChnCode,FTChnName,FTCstCode,FTXshRmk,'
				SET @tSql +=' ROUND((CASE WHEN LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVatable),2),1) = ''5'' AND LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVat),2),1) = ''5'' THEN CAST(SUBSTRING(CONVERT(VARCHAR(22),FCXshVat),1,LEN(CONVERT(VARCHAR(22),FCXshVat))-2) AS DECIMAL(18,2))  ELSE FCXshVat END ),2) AS FCXshVat,'
				SET @tSql +=' ROUND((CASE WHEN LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVatable),2),1) = ''5'' AND LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVat),2),1) = ''5'' THEN CAST(SUBSTRING(CONVERT(VARCHAR(22),FCXshVatable),1,LEN(CONVERT(VARCHAR(22),CONVERT(VARCHAR(22),FCXshVatable)))-2) AS DECIMAL(18,2))  ELSE FCXshVatable END ),2) AS FCXshVatable,'
				SET @tSql +=' ROUND(FCXshAmtNV,2) AS FCXshAmtNV,' 
				SET @tSql +=' ROUND(FCXshRnd,2) AS FCXshRnd,'
				SET @tSql +=' ROUND(FCXshGrand,2) AS FCXshGrand'
				SET @tSql +=' FROM TPSTSalHD HD WITH (NOLOCK)'
				SET @tSql +=' INNER JOIN TCNMBranch Bch WITH (NOLOCK) ON HD.FTBchCode = Bch.FTBchCode'
				SET @tSql +=' LEFT JOIN TCNMChannel_L ChnL WITH (NOLOCK) ON HD.FTChnCode = ChnL.FTChnCode AND ChnL.FNLngID = '''  + CAST(@nLngID  AS VARCHAR(10)) + ''''
				...
				SET @tSql +=' SELECT '''+ @nComName + ''' AS FTComName,'''+ @tRptCode +''' AS FTRptCode, '''+ @tUsrSession +''' AS FTUsrSession,'	
				SET @tSql +=' 1 AS FNAppType,HD.FTBchCode,BchL.FTBchName,HD.FTPosCode,CONVERT(DATE,FDXshDocDate,208) AS FDXshDocDate,'
				SET @tSql +=' HD.FTXshDocNo,FTXshRefInt,'''' AS FTChnCode,'''' AS FTChnName,FTCstCode,FTXshRmk,'
				SET @tSql +=' ROUND((CASE WHEN LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVatable),2),1) = ''5'' AND LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVat),2),1) = ''5'' THEN CAST(SUBSTRING(CONVERT(VARCHAR(22),FCXshVat),1,LEN(CONVERT(VARCHAR(22),FCXshVat))-2) AS DECIMAL(18,2))  ELSE FCXshVat END ),2) AS FCXshVat,'
				SET @tSql +=' ROUND((CASE WHEN LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVatable),2),1) = ''5'' AND LEFT(RIGHT(CONVERT(VARCHAR(22),FCXshVat),2),1) = ''5'' THEN CAST(SUBSTRING(CONVERT(VARCHAR(22),FCXshVatable),1,LEN(CONVERT(VARCHAR(22),CONVERT(VARCHAR(22),FCXshVatable)))-2) AS DECIMAL(18,2))  ELSE FCXshVatable END ),2) AS FCXshVatable,'
				SET @tSql +=' ROUND(FCXshAmtNV,2) AS FCXshAmtNV,' 
				SET @tSql +=' ROUND(FCXshRnd,2) AS FCXshRnd,'
				SET @tSql +=' ROUND(FCXshGrand,2) AS FCXshGrand'
				SET @tSql +=' FROM TVDTSalHD HD WITH (NOLOCK)'
				SET @tSql +=' INNER JOIN TCNMBranch Bch WITH (NOLOCK) ON HD.FTBchCode = Bch.FTBchCode'
				SET @tSql +=' LEFT JOIN TCNMShop SHP WITH (NOLOCK) ON HD.FTShpCode = SHP.FTShpCode AND HD.FTBchCode = SHP.FTBchCode'
				SET @tSql +=' LEFT JOIN TCNMBranch_L BchL WITH (NOLOCK) ON HD.FTBchCode = BchL.FTBchCode AND BchL.FNLngID = '''  + CAST(@nLngID  AS VARCHAR(10)) + ''''
				SET @tSql +=' WHERE HD.FTXshStaDoc = ''1'''
				SET @tSql += @tSql2
		SET @tSql +=' ) AS SalSrc (FTComName,FTRptCode,FTUsrSession,FNAppType,FTBchCode,FTBchName,FTPosCode,FDXshDocDate,FTXshDocNo,FTXshRefInt,FTChnCode,FTChnName,FTCstCode,FTXshRmk,'
					 SET @tSql +=' FCXshVat,FCXshVatable,FCXshAmtNV,FCXshRnd,FCXshGrand'
					SET @tSql +=' ) ON SalTGT.FTComName = SalSrc.FTComName AND'
			  			 SET @tSql +=' SalTGT.FTRptCode = SalSrc.FTRptCode AND'
						 SET @tSql +=' SalTGT.FTUsrSession = SalSrc.FTUsrSession AND'
   						 SET @tSql +=' SalTGT.FNAppType = SalSrc.FNAppType AND'
						 SET @tSql +=' SalTGT.FTBchCode = SalSrc.FTBchCode AND'
						 SET @tSql +=' SalTGT.FTPosCode = SalSrc.FTPosCode AND'
						 SET @tSql +=' SalTGT.FTXshDocNo = SalSrc.FTXshDocNo'
		SET @tSql +=' WHEN NOT MATCHED' 
		SET @tSql +=' THEN INSERT'
			SET @tSql +=' ('
				SET @tSql +=' FTComName,FTRptCode,FTUsrSession,FNAppType,FTBchCode,FTBchName,FTPosCode,FDXshDocDate,FTXshDocNo,FTXshRefInt,FTChnCode,FTChnName,FTCstCode,FTXshRmk,' 
				SET @tSql +=' FCXshVat,FCXshVatable,FCXshAmtNV,FCXshRnd,FCXshGrand'
			SET @tSql +=' )'
		SET @tSql +=' VALUES'
			SET @tSql +=' ('
...
-- โดย			 : เจมส์
-- เวอร์ชั่น		: 01.01.00
-- พี่เอ็มปรับสโตลเรื่อง ตรวจนับสต๊อก คลัง VD แต่ไม่ใช่สินค้าที่อยู๋ในพาโนแกรม

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'STP_DOCxAdjustStockPrc')and OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
DECLARE @tStaAlwCostAmt varchar(1)	-- 06.05.00 --
DECLARE @tPosTo varchar(5)	-- 06.07.00 --
/*---------------------------------------------------------------------
Document History
Version		Date			User	Remark
...
06.04.00	29/12/2021		Em		ยก SC Home มาแทน
06.05.00	17/01/2022		Em		[KPC] เพิ่มการตรวจสอบ Option CostAmt
06.06.00	21/06/2022		Em		[KPC] แก้ไขเรื่องการตรวจนับสต๊อก Vending
06.07.00	19/09/2022		Em		[KPC] แก้ไขเรื่องการตรวจนับสต๊อกคลัง Vending ที่เป็นการตรวจนับทั่วไป
----------------------------------------------------------------------*/
BEGIN TRY
	...
	SELECT TOP 1 @tWahType = ISNULL(WAH.FTWahStaType,''),
	 ...
	 @tStaAlwCostAmt = ISNULL(WAH.FTWahStaAlwCostAmt,''),	-- 06.05.00 --
	 @tPosTo = ISNULL(HD.FTAjhPosTo,''),	-- 06.07.00 --
	...
	INNER JOIN TCNMWaHouse WAH WITH(NOLOCK) ON HD.FTAjhWhTo = WAH.FTWahCode AND HD.FTAjhBchTo = WAH.FTBchCode	-- 06.06.00 --
	...
	BEGIN
		...
		BEGIN
			--IF @tWahType = '6'	-- 4. --
			IF @tWahType = '6' AND @tPosTo != ''	-- 06.07.00 --
				BEGIN
					...
					SET FCStkQty= ISNULL(DT.FCAjdUnitQtyC1,0) -- 06.06.00 --
					,FDLastUpdOn = GETDATE()
					,FTLastUpdBy = @ptWho
...
BEGIN CATCH
	...
	-- 06.07.00 --
	INSERT INTO TSysPrcLog(FTComName, FTUsrCode, FTStkDocNo, FDErrDate, FTerrTime, FNErrNo, FNErrSeverity, FNErrState, FTErrFunction, FNErrLine, FTErrMsg)
	VALUES('ISSUE Adjust Stock', @ptWho, @ptDocNo, CONVERT(VARCHAR(10),GETDATE(),121), CONVERT(VARCHAR(8),GETDATE(),108), 0, 0, 0, 'STP_DOCxAdjustStockPrc', 0, ERROR_MESSAGE())
	-- 06.07.00 --
END CATCH
GO
...
-- โดย			 : เจมส์
-- เวอร์ชั่น		: 02.01.00
-- พี่หนุ่ย เพิ่มการดึงประเภทการปรับราคา baseprice,priceoff ลงฟิวส์ FTXphDocType

/****** Object:  StoredProcedure [dbo].[SP_RPTxAdjPrice]    Script Date: 25/11/2022 10:08:05 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[SP_RPTxAdjPrice]') AND type in (N'P', N'PC'))
	DROP PROCEDURE SP_RPTxAdjPrice
GO
CREATE PROCEDURE SP_RPTxAdjPrice
--ALTER PROCEDURE [dbo].[SP_RPTxAdjPrice] 
	@pnLngID INT , 
	...
	--@ptBchT Varchar(5),
	--สินค้า
	@ptProductCodeF Varchar(5),
	@ptProductCodeT Varchar(5),
	--หน่วยสินค้า
	@ptPdtUnitF Varchar(5),
	@ptPdtUnitT Varchar(5),
	--กลุ่มราคาที่มีผล
	@ptEffectivePriceGroupF Varchar(5),
	@ptEffectivePriceGroupT Varchar(5),
	--วันที่เอกสาร
	@ptDocDateF VARCHAR(50),
	@ptDocDateT VARCHAR(50),
	--วันที่มีผล
	@ptEffectiveDateF VARCHAR(50),
	@ptEffectiveDateT VARCHAR(50),

	@FNResult INT OUTPUT	
AS
BEGIN TRY
---Last Modify 25/11/2022
---V.02.00.00 
---เพิ่ม ประเภทการปรับราคา Baserice หรือ Priceoff
---FTXphDocType ประเภทราคา 1:BasePrice(FTPghStaAdj=1), 2:Price Off
	DECLARE @tSQL VARCHAR(MAX)
	...
	--DECLARE @tBchT Varchar(5)
	--สินค้า
	DECLARE @tProductCodeF Varchar(5)
	DECLARE @tProductCodeT Varchar(5)
	--หน่วยสินค้า
	DECLARE @tPdtUnitF Varchar(5)
	DECLARE @tPdtUnitT Varchar(5)
	--กลุ่มราคาที่มีผล
	DECLARE @tEffectivePriceGroupF Varchar(5)
	DECLARE @tEffectivePriceGroupT Varchar(5)
	--วันที่เอกสาร
	DECLARE @tDocDateF VARCHAR(50)
	DECLARE @tDocDateT VARCHAR(50)
	--วันที่มีผล
	DECLARE @tEffectiveDateF VARCHAR(50)
	DECLARE @tEffectiveDateT VARCHAR(50)
	...
	--หน่วยสินค้า
	SET @tPdtUnitF = @ptPdtUnitF
	SET @tPdtUnitT = @ptPdtUnitT
	--กลุ่มราคาที่มีผล
	SET @tEffectivePriceGroupF = @ptEffectivePriceGroupF
	SET @tEffectivePriceGroupT = @ptEffectivePriceGroupT
	--วันที่เอกสาร
	SET @tDocDateF = @ptDocDateF
	SET @tDocDateT = @ptDocDateT
	--วันที่มีผล
	SET @tEffectiveDateF = @ptEffectiveDateF
	SET @tEffectiveDateT = @ptEffectiveDateT
	...
	--IF (@tBchF <> '' AND @tBchT <> '')
	--	BEGIN
	--		SET @tSQL_Filter += ' AND  ( (SHD.FTBchCode BETWEEN ''' + @tBchF + ''' AND ''' + @tBchT + ''') OR (SHD.FTBchCode BETWEEN ''' + @tBchT + ''' AND ''' + @tBchF + ''') ) '
	--	END

	IF (@ptBchL <> '')
	BEGIN
		SET @tSQL_Filter +=' AND ADJP_HD.FTBchCode IN (' + @ptBchL + ')'
	END

	/*===== Filter สินค้า =============================================*/
	--เช็ค Parameter จากสินค้า
	IF(@tProductCodeF = '' OR @tProductCodeF = NULL)
		BEGIN 
			SET @tProductCodeF = ''
	    END
	ELSE IF(@tProductCodeF <> '')
	   BEGIN 
		   SET @tProductCodeF = @tProductCodeF
	   END

	--เช็ค Parameter ถึงสินค้า
	IF(@tProductCodeT = '' OR @tProductCodeT = NULL)
	   BEGIN 
		   SET @tProductCodeT = ''
	   END
	ELSE IF(@tProductCodeT <> '')
	   BEGIN 
		   SET @tProductCodeT = @tProductCodeT
	   END

	IF (@tProductCodeF <> '' AND @tProductCodeT <> '')
		BEGIN
			SET @tSQL_Filter += ' AND  ( (ADJP_DT.FTPdtCode BETWEEN ''' + @tProductCodeF + ''' AND ''' + @tProductCodeT + ''') OR (ADJP_DT.FTPdtCode BETWEEN ''' + @tProductCodeT + ''' AND ''' + @tProductCodeF + ''') ) '
		END

	/*===== Filter หน่วยสินค้า =============================================*/
	--เช็ค Parameter จากหน่วยสินค้า
	IF(@tPdtUnitF = '' OR @tPdtUnitF = NULL)
		BEGIN 
			SET @tPdtUnitF = ''
	    END
	ELSE IF(@tPdtUnitF <> '')
	   BEGIN 
		   SET @tPdtUnitF = @tPdtUnitF
	   END

	--เช็ค Parameter ถึงหน่วยสินค้า
	IF(@tPdtUnitT = '' OR @tPdtUnitT = NULL)
	   BEGIN 
		   SET @tPdtUnitT = ''
	   END
	ELSE IF(@tPdtUnitT <> '')
	   BEGIN 
		   SET @tPdtUnitT = @tPdtUnitT
	   END

	IF (@tPdtUnitF <> '' AND @tPdtUnitT <> '')
		BEGIN
			SET @tSQL_Filter += ' AND  ( (ADJP_DT.FTPunCode BETWEEN ''' + @tPdtUnitF + ''' AND ''' + @tPdtUnitT + ''') OR (ADJP_DT.FTPunCode BETWEEN ''' + @tPdtUnitT + ''' AND ''' + @tPdtUnitF + ''') ) '
		END

	/*===== Filter กลุ่มราคาที่มีผล =============================================*/
	--เช็ค Parameter จากกลุ่มราคาที่มีผล
	IF(@tEffectivePriceGroupF = '' OR @tEffectivePriceGroupF = NULL)
		BEGIN 
			SET @tEffectivePriceGroupF = ''
	    END
	ELSE IF(@tEffectivePriceGroupF <> '')
	   BEGIN 
		   SET @tEffectivePriceGroupF = @tEffectivePriceGroupF
	   END

	--เช็ค Parameter ถึงกลุ่มราคาที่มีผล
	IF(@tEffectivePriceGroupT = '' OR @tEffectivePriceGroupT = NULL)
	   BEGIN 
		   SET @tEffectivePriceGroupT = ''
	   END
	ELSE IF(@tEffectivePriceGroupT <> '')
	   BEGIN 
		   SET @tEffectivePriceGroupT = @tEffectivePriceGroupT
	   END

	IF (@tEffectivePriceGroupF <> '' AND @tEffectivePriceGroupT <> '')
		BEGIN
			SET @tSQL_Filter += ' AND  ( (ADJP_HD.FTPplCode BETWEEN ''' + @tEffectivePriceGroupF + ''' AND ''' + @tEffectivePriceGroupT + ''') OR (ADJP_HD.FTPplCode BETWEEN ''' + @tEffectivePriceGroupT + ''' AND ''' + @tEffectivePriceGroupF + ''') ) '
		END
	...
	IF (@tDocDateF <> '' AND @tDocDateT <> '')
		BEGIN
			SET @tSQL_Filter += ' AND ( (CONVERT(VARCHAR(10),ADJP_HD.FDXphDocDate,121) BETWEEN ''' + @tDocDateF + ''' AND ''' + @tDocDateT + ''') OR (CONVERT(VARCHAR(10),ADJP_HD.FDXphDocDate,121) BETWEEN ''' + @tDocDateT + ''' AND ''' + @tDocDateF + ''') ) '
		END

	/*===== Filter วันที่มีผล =============================================*/
	--เช็ค Parameter จากวันที่
	IF(@tEffectiveDateF = '' OR @tEffectiveDateF = NULL)
		BEGIN 
			SET @tEffectiveDateF = ''
	    END
	ELSE IF(@tEffectiveDateF <> '')
	   BEGIN 
		   SET @tEffectiveDateF = @tEffectiveDateF
	   END

     --เช็ค Parameter ถึงวันที่
	IF(@tEffectiveDateT = '' OR @tEffectiveDateT = NULL)
	   BEGIN 
		   SET @tEffectiveDateT = ''
	   END
	ELSE IF(@tEffectiveDateT <> '')
	   BEGIN 
		   SET @tEffectiveDateT = @tEffectiveDateT
	   END
	IF (@tEffectiveDateF <> '' AND @tEffectiveDateT <> '')
		BEGIN
			SET @tSQL_Filter += ' AND ( (CONVERT(VARCHAR(10),ADJP_HD.FDXphDStart,121) BETWEEN ''' + @tEffectiveDateF + ''' AND ''' + @tEffectiveDateT + ''') OR (CONVERT(VARCHAR(10),ADJP_HD.FDXphDStart,121) BETWEEN ''' + @tEffectiveDateT + ''' AND ''' + @tEffectiveDateF + ''') ) '
		END

	DELETE FROM TRPTAdjPriceTmp WITH (ROWLOCK) WHERE FTComName = '' + @tComName + '' AND FTRptCode = '' + @tRptCode + '' AND FTUsrSession = '' + @tUsrSession + ''

	SET @tSQL = '
		INSERT INTO TRPTAdjPriceTmp 
		(FTComName,FTRptCode,FTUsrSession,FTPdtCode,FTPdtName,FTPunCode,FTPunName,FDXphDStart,FDXphDStop,FTXphTStart,FTXphTStop,FCXpdPriceRet,FTPplCode,FTPplName,
		FTXphDocNo,FTXphDocType,FDXphDocDate)
	'
	...
	SET @tSQL += '
		SELECT 
			...
			A.FTPplCode,
			A.FTPplName,
			A.FTXphDocNo,A.FTXphDocType,
			A.FDXphDocDate
		FROM 
		(
			SELECT DENSE_RANK() OVER(
               ORDER BY ADJP_DT.FTPdtCode ASC) FNRowID, 
               ADJP_DT.FTPdtCode, 
               PDTL.FTPdtName, 
               ADJP_DT.FTPunCode, 
               PUNL.FTPunName, 
               ...
               CONVERT(VARCHAR(10), ADJP_HD.FDXphDStop, 121) AS FDXphDStop, 
               ADJP_HD.FTXphTStart, 
               ADJP_HD.FTXphTStop, 
               ADJP_DT.FCXpdPriceRet, 
               ADJP_HD.FTPplCode, 
               PL.FTPplName, 
               ADJP_HD.FTXphDocNo, FTXphDocType, 
               CONVERT(VARCHAR(10), ADJP_HD.FDXphDocDate, 121) AS FDXphDocDate
			FROM TCNTPdtAdjPriDT ADJP_DT
            LEFT JOIN TCNTPdtAdjPriHD ADJP_HD WITH(NOLOCK) ON ADJP_HD.FTXphDocNo = ADJP_DT.FTXphDocNo
            LEFT JOIN TCNMPdtUnit_L PUNL WITH(NOLOCK) ON ADJP_DT.FTPunCode = PUNL.FTPunCode AND PUNL.FNLngID = '''+ CAST(@nLngID AS VARCHAR(10)) +'''
            LEFT JOIN TCNMPdt_L PDTL WITH(NOLOCK) ON ADJP_DT.FTPdtCode = PDTL.FTPdtCode AND PDTL.FNLngID = '''+ CAST(@nLngID AS VARCHAR(10)) +'''
            --LEFT JOIN TCNMPdtBar PBAR WITH(NOLOCK) ON ADJP_DT.FTPdtCode = PBAR.FTPdtCode AND ADJP_DT.FTPunCode = PBAR.FTPunCode
            LEFT JOIN TCNMPdtPriList_L PL WITH(NOLOCK) ON ADJP_HD.FTPplCode = PL.FTPplCode AND PL.FNLngID = '''+ CAST(@nLngID AS VARCHAR(10)) +'''
			WHERE 1 = 1
	'
	...
	SET @tSQL += '
		...
		ORDER BY A.FTPdtCode DESC, A.FTPunCode DESC, A.FDXphDocDate DESC
	'
